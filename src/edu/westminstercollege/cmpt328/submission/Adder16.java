package edu.westminstercollege.cmpt328.submission;

import edu.westminstercollege.cmpt328.logic.*;

/*
 * Adder16 class created for CMPT-328 Architecture
 * @Author Nick Foster
 */

public class Adder16 {
	// Inputs to the circuit: InputLines
		public final InputMultiLine A = new InputMultiLine(16);
		public final InputMultiLine B = new InputMultiLine(16);
		public final InputLine Cin = new InputLine();
		Adder4[] adders = new Adder4[] { new Adder4(), new Adder4(), new Adder4(), new Adder4() };

		// Outputs of the circuit: Lines (initialize in constructor)
		public final MultiLine SUM;
		public final Line Cout;

		public Adder16() {
			int i = 0;
			adders[i].A.connectAll(A.lineRange(0, 4));
			adders[i].B.connectAll(B.lineRange(0, 4));
			adders[i].Cin.connect(Cin);

			++i;
			adders[i].A.connectAll(A.lineRange(4, 8));
			adders[i].B.connectAll(B.lineRange(4, 8));
			adders[i].Cin.connect(adders[i - 1].Cout);

			++i;
			adders[i].A.connectAll(A.lineRange(8, 12));
			adders[i].B.connectAll(B.lineRange(8, 12));
			adders[i].Cin.connect(adders[i - 1].Cout);

			++i;
			adders[i].A.connectAll(A.lineRange(12, 16));
			adders[i].B.connectAll(B.lineRange(12, 16));
			adders[i].Cin.connect(adders[i - 1].Cout);

			SUM = MultiLine.join(adders[0].SUM, adders[1].SUM, adders[2].SUM, adders[3].SUM);
			Cout = adders[3].Cout;
		}
}
